...
import { isCropMeaningful, normalizeRotation } from './page-geometry.js';

// Re-exported so the modules that already reach for them here keep working, and so there is
// one obvious home for "page geometry" regardless of which half of it you need.
export { isCropMeaningful, normalizeRotation };
...
/**
 * Where to put something so that it covers `rect` once the page's /Rotate is applied.
 *
 * A viewer rotates the page clockwise by /Rotate, so anything drawn on it has to be rotated
 * the other way to come out upright — and rotating a rectangle about a corner moves it, so the
 * corner to anchor on changes with the angle. Both fall out of the same question: which corner
 * of the drawn box ends up at the content rect's origin once the rotation is applied.
 *
 * Returns the anchor point, the size of the box in its own unrotated frame, and the angle to
 * rotate by. pdf-lib rotates about the point it is given, which is exactly this anchor.
 */
export function placeInContentRect(rect, rotation) {
  ...
  return { anchor: { x: rect.x, y: rect.y }, width: rect.width, height: rect.height, rotate: 0 };
}

/**
 * Map a point in a placed box's own frame — origin at the anchor, y up, unrotated — into
 * content space.
 *
 * Needed because text is drawn from its baseline rather than from a corner: the baseline start
 * is a point inside the box, and it has to travel through the same rotation as the box did.
 */
export function localToContent(placement, rotation, lx, ly) {
  const { anchor } = placement;
  const angle = normalizeRotation(rotation);
  if (angle === 90) return { x: anchor.x - ly, y: anchor.y + lx };
  if (angle === 180) return { x: anchor.x - lx, y: anchor.y - ly };
  if (angle === 270) return { x: anchor.x + ly, y: anchor.y - lx };
  return { x: anchor.x + lx, y: anchor.y + ly };
}
