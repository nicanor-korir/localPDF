...
// Say hello before doing anything.
//
// The page starts a watchdog when it creates this worker, because a worker that never comes up
// is otherwise indistinguishable from one still thinking: no message, no error, and a progress
// overlay that spins until the tab is closed. `onerror` covers a script that fails to parse,
// but not a script that never arrives at all — which is what happens if the asset is missing
// from the cache with no network to fall back to. This message is the proof of life that turns
// that silence into the main-thread fallback instead. It is posted at module scope, so it is
// sent the moment the script evaluates and long before any job is looked at.
post({ type: 'ready' });
