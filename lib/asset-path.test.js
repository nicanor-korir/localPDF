...
describe('assetUrl', () => {
  it('resolves from the app root, not the current page', () => {
    ...
    // /split/merge-worker.js on the split route, which 404s. runMerge treats a worker that
    ...
    expect(assetUrl('merge-worker.js')).toBe('/merge-worker.js');
    expect(assetUrl('pdf.worker.min.js')).toBe('/pdf.worker.min.js');
  });

  it('never returns a relative URL', () => {
    for (const name of ['merge-worker.js', 'pdf.worker.min.js']) {
