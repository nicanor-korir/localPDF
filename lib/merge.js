...
import { createFontCache, drawOverlays } from './draw-overlays.js';
import { groupOverlays } from './overlays.js';
import { isBlank } from './pages.js';
import {
  A4_HEIGHT,
  A4_WIDTH,
  applyPageTransform,
...
async function appendImage(merged, source, page, compressImage, box, preset) {
  ...
  const frame = { x: (pageW - drawW) / 2, y: pageH - drawH, width: drawW, height: drawH };
  out.drawImage(image, frame);
  // The overlay was placed on the picture, not on the paper around it.
  return { page: out, frame };
}
...
export async function mergeDocuments(pages, options = {}) {
  const { sources = [], compressImage, onProgress, title, signal, overlays = [] } = options;
  ...
  // Anything the user added on top of a page. Grouped once so the loop below is a lookup, and
  // an empty map means the whole feature costs a merge with no overlays nothing at all.
  const overlaysByPage = groupOverlays(overlays);
  // A blank page has no source to take a size from, so it borrows the page before it. That is
  // what "insert a blank page" means in a document of Letter pages — another Letter page, not
  // a stray A4 one.
  let lastPageSize = null;
  const fontCache = createFontCache(merged);
  const overlayNotes = [];
  // Each image source is embedded at most once, however many overlays point at it.
  const overlayImages = new Map();
  const embedOverlayImage = async (fileId) => {
    if (overlayImages.has(fileId)) return overlayImages.get(fileId);
    const source = byId.get(fileId);
    let image = null;
    if (source && source.type !== 'application/pdf') {
      try {
        image = await embed(merged, await compressImage(source.file, { preset }));
      } catch (err) {
        console.error('Could not embed an added image:', source.name, err);
      }
    }
    overlayImages.set(fileId, image);
    return image;
  };
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (isBlank(page)) {
      ...
      const [width, height] = lastPageSize ?? [box.width || A4_WIDTH, box.height || A4_HEIGHT];
      const out = merged.addPage([width, height]);
      const turn = normalizeRotation(page.rotation);
      if (turn) out.setRotation(degrees(turn));
      const added = overlaysByPage.get(page.id);
      if (added) {
        overlayNotes.push(...(await drawOverlays(out, added, { fontCache, embedImage: embedOverlayImage })));
      }
    ...
    try {
      ...
      if (source.type === 'application/pdf') {
        ...
        // Drawn before the page is fitted, while the media box is still exactly the content
        // the user placed the overlay on. Fitting then carries it along with everything else.
        if (added) {
          overlayNotes.push(...(await drawOverlays(copied, added, { fontCache, embedImage: embedOverlayImage })));
        }
        fitPageToBox(copied, box);
        merged.addPage(copied);
        lastPageSize = [copied.getWidth(), copied.getHeight()];
      } else {
        // A page may name its own target size. Redaction uses that: the page it replaces is
        // an image of the original, and it should come out exactly the size it went in.
        const target = page.pageBox ?? box;
        const { page: out, frame } = await appendImage(merged, source, page, compressImage, target, preset);
        lastPageSize = [out.getWidth(), out.getHeight()];
        if (added) {
          overlayNotes.push(
            ...(await drawOverlays(out, added, { fontCache, embedImage: embedOverlayImage, frame })),
          );
        }
      }
    } catch (err) {
  ...
  merged.setProducer('LocalPDF');
  merged.setCreator('LocalPDF');
  ...
  return { bytes, skipped, overlayNotes };
}
