 ...
 *
 * ```javascript
 * { id, pageId, type: 'text', x, y, width, height, text, size, colour }
 * { id, pageId, type: 'image', x, y, width, height, fileId }
 * { id, pageId, type: 'redaction', x, y, width, height }
 * ```
 *
 * A redaction is stored here so it inherits the same undo, reconciliation and dragging as
 * everything else, but it is **not drawn onto the page** the way the others are — see
 * `app/_lib/redact.js` for why removing content and covering it are different jobs.
 *
...
/**
 * Mark a region to be removed.
 *
 * Deliberately carries no colour or style. A redaction is not a black rectangle you can
 * configure; it is a promise that what was underneath is gone, and offering to make it
 * semi-transparent would invite someone to make that promise false.
 */
export function addRedaction(overlays, pageId, box) {
  return [...overlays, { id: generateId(), pageId, type: 'redaction', ...clampBox(box) }];
}
...
export function redactionsForPage(overlays, pageId) {
  return overlays.filter((o) => o.pageId === pageId && o.type === 'redaction');
}

/** The ids of every page carrying at least one redaction. */
export function redactedPageIds(overlays) {
  return new Set(overlays.filter((o) => o.type === 'redaction').map((o) => o.pageId));
}
