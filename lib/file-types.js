...
/**
 * Name the output after the first source, tagged with what was done to it.
 *
 * The suffix is a parameter rather than a constant because every tool produces a file the user
 * has to tell apart from the original in their downloads folder, and calling the output of
 * "organise" a merge is a small lie that costs them a moment every time.
 */
export function buildDownloadName(firstName, suffix = 'merged') {
  const base = (firstName || suffix).replace(/\.[^.]+$/, '').trim();
