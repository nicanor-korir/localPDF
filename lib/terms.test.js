...
describe('what the agreement says', () => {
  ...
  it('discloses the one thing that is stored, and denies the things that are not', () => {
    const storage = TERMS.find((t) => /kept on your device/i.test(t.title));
    expect(storage).toBeDefined();
    expect(storage.body).toMatch(/no cookies/i);
    expect(storage.body).toMatch(/no tracking/i);
    expect(storage.body).toMatch(/no analytics/i);
  });

  it('is short enough that someone might actually read it', () => {
    // A wall of text is how a real disclosure becomes an unread one. Four points, and the
    // whole thing under 150 words.
