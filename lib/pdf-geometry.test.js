...
describe('placeInContentRect and localToContent', () => {
  const rect = { x: 100, y: 200, width: 60, height: 40 };

  /** The four corners of a placed box, mapped back into content space. */
  const corners = (rotation) => {
    const placed = placeInContentRect(rect, rotation);
    return [
      localToContent(placed, rotation, 0, 0),
      localToContent(placed, rotation, placed.width, 0),
      localToContent(placed, rotation, placed.width, placed.height),
      localToContent(placed, rotation, 0, placed.height),
    ];
  };

  it.each([0, 90, 180, 270])('covers exactly the target rect at %i degrees', (rotation) => {
    // Whatever the angle, the drawn box must land on the rect the caller asked for — no
    // sliding off the page, no covering the wrong half of it.
    const points = corners(rotation);
  ...
  it('swaps the box dimensions on a quarter turn', () => {
    expect(placeInContentRect(rect, 90)).toMatchObject({ width: 40, height: 60, rotate: 90 });
    expect(placeInContentRect(rect, 270)).toMatchObject({ width: 40, height: 60 });
    expect(placeInContentRect(rect, 180)).toMatchObject({ width: 60, height: 40 });
  });

  it('puts the local origin at the corner that displays as the bottom-left', () => {
    // At 0 that is simply the rect's own origin.
    expect(localToContent(placeInContentRect(rect, 0), 0, 0, 0)).toEqual({ x: 100, y: 200 });
    // At 180 it is the opposite corner, because the box is drawn back towards it.
    expect(localToContent(placeInContentRect(rect, 180), 180, 0, 0)).toEqual({ x: 160, y: 240 });
  });

  it('normalises an angle it was not expecting', () => {
    expect(placeInContentRect(rect, 450).rotate).toBe(90);
    expect(placeInContentRect(rect, -90).rotate).toBe(270);
  });
});
