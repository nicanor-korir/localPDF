...
describe('buildDownloadName', () => {
  ...
  it('tags the output with the tool that made it', () => {
    // Every tool writes into the same downloads folder, so calling the output of "organise" a
    // merge makes the user open the wrong file.
    expect(buildDownloadName('report.pdf', 'organised')).toBe('report-organised.pdf');
    expect(buildDownloadName('report.pdf', 'extracted')).toBe('report-extracted.pdf');
    expect(buildDownloadName('', 'extracted')).toBe('extracted-extracted.pdf');
  });
});
