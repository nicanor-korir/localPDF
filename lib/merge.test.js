...
import { makeBlankPage } from './pages.js';
import {
  asSource,
  LINK_RECT,
  makeCorruptPdf,
...
describe('mergeDocuments', () => {
  ...
  it('writes its own metadata rather than the library default', async () => {
    ...
    const doc = await reload(out.bytes);
    expect(doc.getProducer()).toBe('LocalPDF');
...
describe('blank pages', () => {
  it('adds an empty page of its own', async () => {
    const source = asSource(await makePdf({ width: 400, height: 600 }), 'doc.pdf');
    const pages = [...pagesOf(source, 1), makeBlankPage()];
    const { bytes } = await mergeDocuments(pages, { sources: [source], compressImage, pageSize: 'original' });

    const out = await reload(bytes);
    expect(out.getPageCount()).toBe(2);
  });

  it('borrows the size of the page before it', async () => {
    // "Insert a blank page" in a document of Letter pages means another Letter page, not a
    // stray A4 one.
    const source = asSource(await makePdf({ width: 612, height: 792 }), 'letter.pdf');
    const pages = [...pagesOf(source, 1), makeBlankPage()];
    const { bytes } = await mergeDocuments(pages, { sources: [source], compressImage, pageSize: 'original' });

    const [first, blank] = (await reload(bytes)).getPages();
    expect(Math.round(blank.getWidth())).toBe(Math.round(first.getWidth()));
    expect(Math.round(blank.getHeight())).toBe(Math.round(first.getHeight()));
  });

  it('falls back to the target size when it comes first', async () => {
    const source = asSource(await makePdf({ width: 400, height: 600 }), 'doc.pdf');
    const pages = [makeBlankPage(), ...pagesOf(source, 1)];
    const { bytes } = await mergeDocuments(pages, { sources: [source], compressImage, pageSize: 'a4' });

    const [blank] = (await reload(bytes)).getPages();
    expect(Math.round(blank.getWidth())).toBe(Math.round(A4_WIDTH));
    expect(Math.round(blank.getHeight())).toBe(Math.round(A4_HEIGHT));
  });

  it('honours a rotation, which is the only transform that means anything on it', async () => {
    const source = asSource(await makePdf({ width: 400, height: 600 }), 'doc.pdf');
    const blank = { ...makeBlankPage(), rotation: 90 };
    const { bytes } = await mergeDocuments([...pagesOf(source, 1), blank], {
      sources: [source],
      compressImage,
      pageSize: 'original',
    });
    expect((await reload(bytes)).getPages()[1].getRotation().angle).toBe(90);
  });

  it('makes a document out of nothing but blank pages', async () => {
    const { bytes } = await mergeDocuments([makeBlankPage(), makeBlankPage()], {
      sources: [],
      compressImage,
    });
    expect((await reload(bytes)).getPageCount()).toBe(2);
  });

  it('does not give a blank page a bookmark of its own', async () => {
    const a = asSource(await makePdf({ pages: 1 }), 'first.pdf');
    const b = asSource(await makePdf({ pages: 1 }), 'second.pdf');
    const { bytes } = await mergeDocuments(
      [...pagesOf(a, 1), makeBlankPage(), ...pagesOf(b, 1)],
      { sources: [a, b], compressImage },
    );

    // Two source documents, two entries. A blank page is not a document, and it must not
    // split the one it sits inside either.
    const outlines = (await reload(bytes)).catalog.lookup(PDFName.of('Outlines'));
    expect(outlines.lookup(PDFName.of('Count')).asNumber()).toBe(2);
  });
});
